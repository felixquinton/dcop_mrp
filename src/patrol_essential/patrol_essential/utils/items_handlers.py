...
For your use case you can use ItemHandler as mother class and create your own
handlers.
"""

import json

# Import for patrol basic use case
...
class ItemHandler:
    """
    This class is a mother class to handle item awarded in a bundle. This is
    only for static information access and not
    planning.
    """
    ...
    def create_goal_msg(self, goal_id='default_goal_id'):  # TODO to remove
        """
        This function create a goal_msg for this item handler.
        Useful if item is already a valid action
...
def return_item_handler(item_description_msg):
    """
    Function that returns the corresponding item handler for an item
    description message
...
class AbstractActionItemHandler(ItemHandler):
    ...
    def create_goal_msg(self, goal_id='default_goal_id'):
        return AbstractAction.Goal(goal_id=goal_id,
...
class GoToItemHandler(ItemHandler):
    ...
    def parse_item_description_data(self):
        ...
        try:
            ...
            self.target_position = Point(x=target_position['x'],
                                         y=target_position['y'],
                                         z=target_position['z'])
        except KeyError:
            print('ERROR, impossible to parse ', self.item_type)
