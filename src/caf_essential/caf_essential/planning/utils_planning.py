...
from caf_essential.utils.items_handlers import return_item_handler
import traceback
...
class Plan:
    ...
    def __init__(self, robot_node):
        ...
        self.p_items_hdl = {}  # dict of planned plan_item_hdl (key: item_id, value: plan_item_hdl)
    ...
    def add_action(self, action, p_item_hdl_concerned):
        self.actions[action.goal_id] = PlanActionHandler(action, p_item_hdl_concerned)
    ...
    def execute_next_actions(self):
        """
        Checks actions that can be executed, by example next action by verifying precondition if sequential, or current
