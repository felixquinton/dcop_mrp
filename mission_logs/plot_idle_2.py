...
colors = {"our": 'b', "sheng": 'r', "none": "g", "our+sheng": "m"}
colors_per_pmul = {
...
nb_inst_dic = {"our+sheng": 15, "our": 50, "sheng": 50, "none": 50}
...
for cr in [50, 60, 75]:
        for pmul in ["1", "0", "1", "2"]:
                ...
                end_ind_list = []
                for auction_type in ["our+sheng", "our", "sheng", "none"]:
                ...
                plt.legend()
                plt.savefig(f"ICRA_tests_res/simu_icra_obs_{pmul}_cr_{cr}/mean_idleness_obs_{pmul}_cr_{cr}.pdf")
                plt.clf()
                ...
                for auction_type in ["our+sheng", "our", "sheng", "none"]:
                    ...
                    plt.plot(np.array(list(tota_max_com_i1_k1[cr][auction_type].keys())[:end_ind_com]),
                             ...
                             linewidth=1, alpha=1, label=f"Com range = {cr}, {auction_type} auction", linestyle="dashed", color=colors[auction_type])
                plt.legend()
                plt.savefig(f"ICRA_tests_res/simu_icra_obs_{pmul}_cr_{cr}/max_idleness_obs_{pmul}_cr_{cr}.pdf")
                plt.clf()
                print("Plot done.")
