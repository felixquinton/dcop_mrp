...
nb_inst_dic = {"ourssheng": 30, "ours": 19, "sheng": 21, "no_com": 21}
...
for auction_type in ["ours", "sheng", "no_com"]:
        ...
        first_folder = f"IROS_tests/caylus_small/{auction_type}"
        end_ind_com = 1800
        max_distrib = {}
        ...
        for i in range(nb_inst):
            if auction_type == "no_com" and i in []:
                continue
            if auction_type == "sheng" and i in []:
                continue
            if auction_type == "ours" and i in []:
                continue
            ...
            data_dic = {}
            data_dic[0.0] = {str(w): 1 for w in range(21)}
            data_mean = {}
            data_max = {}
            previous_time = 0.0
            for ind, line in enumerate(data.iterrows()):
                    if round(line[1][1]) < 1800:
...
plt.savefig(f"IROS_tests/caylus_small/mean.pdf")
plt.clf()
...
plt.xlim(left=0, right=1800)
plt.ylim(bottom=0, top=1800)
plt.savefig(f"IROS_tests/caylus_small/max.pdf")
plt.clf()
print("Plot done.")
